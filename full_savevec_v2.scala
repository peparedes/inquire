...
def query( query_s : String , top : Int, filter: String = "", minWords: Int = 5, maxWords : Int = 40)={
	...
	val fw0 = new FileWriter("./foodtype/queryhistory_kmeans.txt",true);
	...
	//      println(sent)
		...
		//try id
		  ...
		  if(numWords >= minWords && numWords <= maxWords &&// minimum words
			...
			if(ids.ncols>ix+1){
				...
				outVectors = outVectors on FMat(magic(?,ix).t);
				count += 1;
		...
		outVectors = outVectors(bestIndex0,?);
	...
	outVectors = outVectors(bestIndex,?);
	//save to txt file
	println("save to txt")
	//val fw1 = new FileWriter("./foodtype/privacy_queryresult_full_top.txt",false);
	val fw1 = new FileWriter("./foodtype/queryresult_kmeans.txt",false);
	val bw1 = new BufferedWriter(fw1);
	val out1 = new PrintWriter(bw1);
	for(i <-0 until top+1){
		println(outResFMat(i).toString+'\t'+outSentMat(i)+'\t'+outURLsSMat(i))
		out1.println(outResFMat(i).toString+'\t'+outSentMat(i)+'\t'+currentTime+'\t'+outURLsSMat(i));
	}
	out1.close();
	saveFMat("./foodtype/vecs_full_top.fmat.lz4",FMat(outVectors))
	//save to hdf5 file
