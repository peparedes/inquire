...
object featurizers {
  ...
  def parseUser(w: IMat, dict: Dict): String = {
    var out = "";
    var prevNum = false;
    for(i <- 0 until w.size) {
      val num = w(i);

      if(i != 0 && num > 0 && !prevNum) {
        out += "-";
      }

      var s = "";
      if(num <= 0) {
        var n = twoComplementToInt(num)(0);
        if(n < 0) {
          n += Int.MinValue;
        }
        s = n + "";
  ...
  def featurizeMoodIDSentence(indir: String, dictdir:String,outdir:String,fileListPath: String): Unit = {
    ...
    val userDict = Dict(loadSBMat("/home/franky/metadata_dicts/users_dict_keys.sbmat.lz4"));
    val userDictVals = loadSBMat("/home/franky/metadata_dicts/users_dict_values.sbmat.lz4");
    ...
    var labels = izeros(5, MaxMb*postPerMb*2); // Dense IMat with (UserId, CurrentMoodId, ditemid, Timestamp, reply_count)
    var usernames = CSMat(1, MaxMb*postPerMb*2);
        ...
        breakable {
	      ...
	      if (moodIdx.nrows==1 && xmlFile(postStart+moodIdx(0,0)) == intIndex ) {
	        ...
	        if (moodid>0 && moodid < 135 && moodid!=50 && moodid!=94) {
              ...
              try {
                val ditemIdx = getBeginEnd(xmlFile(postStart -> postEnd), xmlDict, "ditemid");
	            ditemid = twoComplementToInt(xmlFile(postStart+ditemIdx(0,0)+1))(0);

                val timestampIdx = getBeginEnd(xmlFile(postStart -> postEnd), xmlDict, "event_timestamp");
                timestamp = xmlFile(postStart+timestampIdx(0,0)+1)(0) + Int.MinValue;

                val replycountIdx = getBeginEnd(xmlFile(postStart -> postEnd), xmlDict, "reply_count");
                replycount = twoComplementToInt(xmlFile(postStart+replycountIdx(0,0)+1))(0);


                val userIdx = getBeginEnd(xmlFile(postStart -> postEnd), xmlDict, "user")
                val w = xmlFile((postStart+userIdx(0,0)) -> (postStart+userIdx(0,1)))
                username = parseUser(w, xmlDict);

                // val userhash = ditemid % 100000  + 100000*(timestamp % 10000);
                // val possibleUser = CSMat(usersMat(?, userhash))(0);

                var key = username.replaceAll("[-0]+", "") + "|" + ditemid + "|" + timestamp;
                val uix = userDict(key);
                if(uix >= 0) {
                  username = CSMat(userDictVals(?, uix))(0);
                }

                // if(possibleUser.length > 0 &&
                //   username.replaceAll("[-0]+", "") == possibleUser.replaceAll("[-0]+", "") &&
                //   username != possibleUser) {
                //   printf("corrected user %s -> %s\n", username, possibleUser);
                //   username = possibleUser;
                // }
                ...
                //     || username.matches(".*[0-9]-.*")) {
                //     printf("WARNING: have user %s but matched %s\n", username, possibleUser);
                //   }
                // }

              } catch {
                case oob: IndexOutOfBoundsException => {
                  println(s"problem getting metadata for post ${posti} in file ${line}.xml");
                  break;
                }
              }
              ...
              // println(s"labels size is ${labels.nrows} rows by ${labels.ncols} cols");
              // println(s"denseEntryNumber = $denseEntryNumber");

              var nSents = maxi(sentID)(0) + 1;
              var denseNums = irow(denseEntryNumber -> (denseEntryNumber + nSents));

              // println(s"nSents = $nSents");

              // Add to "labels" -> userid, currentmoodId
	          labels(0,denseNums) = userk+nrUsers;
	          labels(1,denseNums) = moodid;
              labels(2,denseNums) = ditemid;
              labels(3,denseNums) = timestamp;
              labels(4,denseNums) = replycount;

              usernames(denseNums) = username;
	          ...
	          if (denseEntryNumber >= MaxMb*postPerMb) {
                ...
                saveSBMat(outdir+"data"+s"$batchNumber"+"_usernames.sbmat.lz4", SBMat(usernames(0 until denseEntryNumber)));
